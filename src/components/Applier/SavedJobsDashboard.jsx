...
const SavedJobsDashboard = () => {
  // Local state for job list, search term, modal, applied jobs, and loading
...
useEffect(() => {
  // Fetch saved jobs for current user and filter out expired ones
  if (!user?.uid) return;
  ...
  try {
    ...
    const matchedJobs = jobSnap.docs
  ...
  return (
    <>
      ...
      <div className="job-list-container">
        ...
        <h1>Stay Ahead with Saved Jobs</h1>
        <p>Return to the roles that caught your eye</p>
          ...
          <div className="job-cards" >
            {filteredJobs.length === 0 ? (
              <div className="no-jobs-message">
                <p>All Saved Jobs are Expired / No Available Saved Jobs</p>
              </div>
            ) : (
              filteredJobs.map(job => (
                <div key={job.id} className="job-card" onClick={() => setSelectedJob(job)}>

                  {/* Badge if already applied */}
